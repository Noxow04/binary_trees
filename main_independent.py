class Tree:
    class Node:
        ...
        def __repr__(self):
            return f"Tree.Node(value:{self.value}, left:{self.left}, right:{self.right})"

    def __init__(self, root_value=None, allow_duplicates=False):
        self._root = Tree.Node(root_value) if root_value else None
    ...
    def iterations(self, value, node=None):
    ...
    def calculate_depth(self, node=None, include_root_in_depth=True):
        if not node:
            if self.root:
                node = self.root
            else:
    ...
    def optimize(self, keep_root=False):
        if keep_root:
    ...
    @classmethod
    def build_from_values(cls, root_value, *values, allow_duplicates=False):
    ...
    @staticmethod
    def _sorted_insert(values, node):
